(function () {
    ...
    function detectExplicitPhotoRequest(text=''){
        ...
        const allow=[/给我(?:看|看看|看一看).{0,12}你/,/(?:给我)?发(?:一|1)?张?.{0,8}(照片|图片|自拍|相片)/,/(?:拍|照)(?:一|1)?张?.{0,8}(照片|图片|自拍|相片)?(?:给我|发我|看看)?/,/^(?:来|发|拍)?(?:一|1)?张?自拍(?:照|照片)?(?:给我|发我|看看)?[吧呀啊。！!？?]*$/,/(?:给我|让我|想)看看你.{0,16}(样子|穿着|衣服|现在|在干嘛)?/,/show me (?:a |your )?(?:photo|picture|selfie)/,/send me (?:a |your )?(?:photo|picture|selfie)/,/take (?:a |your )?(?:photo|picture|selfie)/];
        const denied=deny.some(p=>p.test(normalizedText)); const matched=!denied&&allow.find(p=>p.test(normalizedText));
    ...
    function createTurnContext(messages=[]){const latestUserText=messages.map(m=>typeof m==='string'?m:(m?.contentType==='text'?m.content||'':'')).filter(Boolean).join('\n');const photoRequest=detectExplicitPhotoRequest(latestUserText);return{id:uid(),friendId:null,createdAt:new Date().toISOString(),latestUserText,photoRequest,realImageGenerationAllowed:photoRequest.requested,realImageGenerationConsumed:false};}
    function sanitizeImageActions(actions,ctx){if(!Array.isArray(actions))return actions;return actions.reduce((out,source)=>{if(!source||typeof source!=='object')return out;const action={...source},type=String(action.type||'').toLowerCase();if(!IMAGE_TYPES.has(type)){out.push(action);return out;}const prompt=[action.image_prompt,action.prompt,action.description,action.imageDescription,action.scene,action.content].find(v=>typeof v==='string'&&v.trim());if(!prompt)return out;action.type='image';action.image_prompt=prompt.trim();const generate=ctx?.realImageGenerationAllowed===true&&ctx?.realImageGenerationConsumed!==true;if(generate){action.imageMode='generate';ctx.realImageGenerationConsumed=true;}else action.imageMode='prompt-only';out.push(action);return out;},[]);}
 ...
 async function handleImageAction({action,friend,friendId}){
    ...
    const api={init,getSettings,saveSettings,detectExplicitPhotoRequest,createTurnContext,sanitizeImageActions,joinApiUrl,buildHeaders,buildBody,extractGeneratedImage,normalizeImageApiError,buildCharacterImagePrompt,generateImage,handleImageAction,sendPromptOnlyImageMessage,createPlaceholderMessage,replacePlaceholderMessage,markPlaceholderFailed,retryImageMessage,handleImageClick,openSettings,saveForm,fetchModels,testConnection,savePreset,selectPreset,clearSettings,toggleApiKeyVisibility,modelListUrls};
    window.JrsyImageGeneration=api; if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
}());
